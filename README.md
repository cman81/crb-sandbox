...
* **Passive Phaser 4 / Multi-Socket Node.js Truth:** Server dictates all data layers. 
* **Array-Based Concurrent Seating (Multi-Socket Fixed Loop):** Refactored from single string identifiers (`playerA: null`) to concurrent list arrays (`playerA: []`). Multiple browser tabs, test devices, or ghost connection refreshes can inhabit the same player seat simultaneously.
* **Synchronized Array Broadcasting:** Loops through seat socket lists via `.forEach()` using custom server broadcasters. This guarantees that all matching viewports catch network update triggers (`cardDrawnUpdate`, `discardRecycledUpdate`) instantly and simultaneously.
...
* **`executeDevMulligan` Endpoint:** Merged into Tab 2 button of `DeveloperMode`. Pops hand elements to deck tail, shuffles via random UUID alphanumeric sorting parameters, redeals 6 cards, and invokes array broadcasts to force synchronized FOW canvas repaints.
